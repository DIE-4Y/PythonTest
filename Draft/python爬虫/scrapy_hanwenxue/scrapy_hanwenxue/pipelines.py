...
class ScrapyHanwenxuePipeline(object):
    # 构造方法（初始化对象时执行的方法）
    def __init__(self):
        # 必须使用 w+ 模式打开文件，以便后续进行 读写操作（w+模式，意味既可读，亦可写）
        # 注意：此处打开文件使用的不是 python 的 open 方法，而是 codecs 中的 open 方法
        """保存实词"""
        # self.json_file = codecs.open('./HanWenxue/shici.json', 'w+', encoding='UTF-8')
        """保存古诗"""
        self.json_file = codecs.open('./HanWenxue/poetry.json', 'w+', encoding='UTF-8')
