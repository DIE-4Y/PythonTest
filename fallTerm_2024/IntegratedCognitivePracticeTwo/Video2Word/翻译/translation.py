...
class get_result(object):
    def __init__(self, host, text, src_language, des_language):
        # 应用ID（到控制台获取）
        self.APPID = "ac1587de"
        # 接口APISercet（到控制台机器翻译服务页面获取）
        self.Secret = "OGZhNDU1MjhjMWVkMmQwZDY5ZDYzOTNl"
        # 接口APIKey（到控制台机器翻译服务页面获取）
        self.APIKey = "32852eb534e43fb0db1b87bafe4c1500"
    ...
    def call_url(self):
        ...
        if self.APPID == '' or self.APIKey == '' or self.Secret == '':
            ...
            if status_code != 200:
                ...
                print(type(result2))
                with open("TranslationResult/result.txt", 'w', encoding='utf-8') as fp:
                    fp.write(result2)
...
"""机器翻译接口"""
def translation(text, src_language, des_language):

    host = "itrans.xfyun.cn"
    # 初始化类
    gClass = get_result(host, text, src_language, des_language)
